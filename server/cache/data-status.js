...
// cloud IPs); an external daily runner refreshes that JSON, and its threshold
// is 3 weeks. Recruiting is served from static JSON, but successful refresh
// cadence is monitored externally by the Sentry Cron Monitor. Its file mtime
// measures content changes rather than successful no-op checks, so it keeps
// `alert: false` and no staleness threshold.
const DATASETS = [
  {
    name: "news",
  ...
  },
  {
    ...
    fallbackFile: "data/asu_transfers_fallback.json",
  ...
  },
  {
    name: "alumni",
    cacheKey: "asu_alumni",
    fallbackFile: "data/asu_alumni_fallback.json",
    staleAfterMs: 21 * DAY_MS,
    alert: true,
  },
  {
    name: "recruiting",
    staticFile: "asu_hockey_data.json",
    staleAfterMs: null,
    alert: false,
  },
];
...
function resolveCacheKey(dataset) {
  ...
  return typeof dataset.cacheKey === "function"
    ? dataset.cacheKey()
    : dataset.cacheKey;
}
...
function readFallbackMeta(relPath) {
  ...
  try {
    ...
    console.error(
      `[Data Status] Failed to read fallback meta for ${relPath}:`,
      error.message,
    );
    return null;
  }
  ...
  return DATASETS.map((dataset) => {
    ...
    if (dataset.staticFile) {
      ...
      return {
        ...
        ageMs: meta.ageMs,
        status: statusFor(meta.ageMs, dataset.staleAfterMs),
      };
    }
    ...
    const fallbackMeta = dataset.fallbackFile
      ? readFallbackMeta(dataset.fallbackFile)
      : null;
    ...
    if (
      cacheMeta &&
      (!fallbackMeta ||
        (cacheMeta.ageMs ?? Infinity) <= (fallbackMeta.ageMs ?? Infinity))
    ) {
      source = "cache";
      meta = {
        key: cacheMeta.key,
        timestamp: cacheMeta.timestamp,
        ageMs: cacheMeta.ageMs,
      };
    } else if (fallbackMeta) {
      source = "fallback";
      meta = {
        file: fallbackMeta.file,
        timestamp: fallbackMeta.timestamp,
        ageMs: fallbackMeta.ageMs,
      };
    }
...
function getCooldownStatus() {
  try {
    ...
    console.error(
      "[Data Status] Failed to list cooldown markers:",
      error.message,
    );
    return [];
  }
