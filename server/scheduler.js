...
function withRetry(label, fn) {
  return Promise.resolve()
    ...
    .catch((err) => {
      console.error(
        `[Scheduler] ${label} failed; retrying in ${RETRY_DELAY_MS / 60000} min`,
        err,
      );
      Sentry.captureException(err, {
        tags: { component: "scheduler", task: label },
      });
      setTimeout(() => {
        Promise.resolve()
          .then(fn)
          .then(
            ...
            (retryErr) => {
              console.error(
                `[Scheduler] ${label} retry also failed.`,
                retryErr,
              );
...
async function refreshFrequentData() {
  console.log(
    "[Scheduler] Starting frequent data refresh (News & Schedule)...",
  );
  await Promise.all([
    fetchNewsData(),
    fetchScheduleData(),
...
async function refreshPostGameData() {
  console.log(
    "[Scheduler] Starting post-game data refresh (Schedule & Stats)...",
  );
  await Promise.all([
    fetchNewsData(),
...
async function refreshDailyData() {
  console.log(
    "[Scheduler] Starting daily data refresh (Roster, Alumni, Transfers)...",
  );
  await Promise.all([
    scrapeCHNRoster(),
    scrapeAlumniData(),
...
function startScheduler() {
  ...
  cron.schedule("0 2,3,4,5,6 * * 6,0", () => {
    console.log(
      "[Cron] Triggering weekend post-game refresh (Schedule & Stats)...",
    );
