...
async function scrapeTransferData() {
  ...
  if (shouldUseFallbackOnly()) {
    ...
    if (fallback) {
      console.log(
        "[Transfer Scraper] Returning bundled fallback transfer data",
      );
      return fallback;
    }
  }
  ...
  if (transferPromise) {
    console.log(
      "[Transfer Scraper] Scrape already in progress. Returning shared promise.",
    );
  ...
  transferPromise = (async () => {
    try {
      ...
      $(
        ...
        let direction = "";
        if (
          sectionText.includes("joining") ||
          sectionText.includes("incoming")
        ) {
          direction = "incoming";
        } else if (
      ...
      incoming.forEach((t) =>
        console.log(
          `  [JOINING] ${t.playerName} (${t.position}) from ${t.team}`,
        ),
      );
      outgoing.forEach((t) =>
      ...
      if (!reportScrapeHealth("transfers", { totalTransfers: total })) {
        ...
        if (fallback) {
          console.log(
            "[Transfer Scraper] Empty result; returning bundled fallback",
          );
          return fallback;
        }
        return result;
      ...
      console.error(
        "[Transfer Scraper] Error scraping transfers:",
        error.message,
      );
      ...
      try {
        ...
        if (stale) {
          console.log(
            "[Transfer Scraper] Returning stale cached data due to error",
          );
          return stale;
        }
      } catch (e) {
      ...
      if (fallback) {
        console.log(
          "[Transfer Scraper] Returning bundled fallback transfer data after scrape error",
        );
        return fallback;
      }
