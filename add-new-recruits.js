...
const NEW_PLAYERS = [
  {
    ...
    season: "2027-2028",
    position: "D",
    player_link: "https://www.eliteprospects.com/player/863782/henry-chmiel",
  },
];
...
async function scrapeEPProfile(url) {
  ...
  const photoSrc =
    $('img[src*="files.eliteprospects.com/layout/players"]')
      .first()
      .attr("src") || $('[class*="ProfileImage"] img').first().attr("src");
  if (photoSrc) {
    player_photo = photoSrc.startsWith("http")
      ? photoSrc
      : `https://www.eliteprospects.com${photoSrc}`;
  }
  ...
  const bio = {};
  $(
    '[class*="PlayerInfo"] [class*="InfoItem"], .ep-list__item, dl dt, dl dd',
  ).each((_, el) => {
    const text = $(el).text().trim();
    ...
    if (label.includes("shoots") || label.includes("catches"))
      bio.shoots = text;
  ...
  $('table tr, [class*="PlayerInfo"] li, [class*="ep-td"]').each((_, el) => {
    ...
    if (cells.length >= 2) {
      ...
      if ((key.includes("shoots") || key.includes("catches")) && val)
        bio.shoots = val;
      ...
      if ((key.includes("birthplace") || key.includes("place of birth")) && val)
        bio.birthplace = val;
    }
  });
...
async function main() {
  ...
  for (const player of NEW_PLAYERS) {
    ...
    try {
      ...
      console.log(
        `  height: ${profile.height || "?"}, weight: ${profile.weight || "?"}, shoots: ${profile.shoots || "?"}`,
      );
      console.log(
        `  birth_year: ${profile.birth_year || "?"}, age: ${profile.age || "?"}, birthplace: ${profile.birthplace || "?"}`,
      );
    } catch (e) {
