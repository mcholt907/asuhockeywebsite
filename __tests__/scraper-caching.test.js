...
describe("scrapeCHNRoster — SWR caching", () => {
  ...
  test("returns stale roster immediately when cache is expired, without blocking on live scrape", async () => {
    ...
    getFromCache.mockReturnValueOnce(null).mockReturnValueOnce(staleRoster);
...
describe("scrapeCHNScheduleLinks", () => {
  test("returns date-keyed linkMap plus npi/krach metadata", async () => {
    ...
    expect(result.linkMap).toEqual({
      "2025-10-03": {
        box_link:
          "https://www.collegehockeynews.com/box/final/20251003/psu/asu/",
        metrics_link:
          "https://www.collegehockeynews.com/box/metrics.php?gd=110368",
      },
      "2025-10-10": {
        box_link:
          "https://www.collegehockeynews.com/box/final/20251010/ndm/asu/",
        metrics_link:
          "https://www.collegehockeynews.com/box/metrics.php?gd=110371",
      },
    });
...
describe("scrapeUSCHORecord", () => {
  ...
  test("returns null when Inertia JSON is missing expected record sub-fields", async () => {
    const page = {
      props: {
        content: { record: { total: { wins: 5, losses: 3, ties: 0 } } },
      },
    }; // conf/home/road absent
    requestWithRetry.mockResolvedValueOnce({
