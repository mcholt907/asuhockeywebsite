...
import { SEASON_SHORT } from "../config/season";
import "./Stats.css";
...
function Stats() {
  ...
  const getLeaders = (data, key, count = 3) => {
    ...
    return data
      .filter((p) => {
        ...
        return (
          p && p[actualKey] !== undefined && !isNaN(parseFloat(p[actualKey]))
        );
      })
      .sort((a, b) => {
    ...
    <div className="leader-card">
      ...
      <div className="leader-list">
        {data.map((player, idx) => {
          ...
          let name =
            player.Player || player["Name, Yr"] || player.Name || "Unknown";
          if (name.includes(",")) {
  ...
  const skaterHeaders =
    stats.skaters.length > 0 ? Object.keys(stats.skaters[0]) : [];
  const goalieHeaders =
    stats.goalies.length > 0 ? Object.keys(stats.goalies[0]) : [];
  ...
  return (
    <div className="page-container stats-page">
      <Helmet>
        ...
        <meta
          name="description"
          content={`ASU Sun Devils Men's Hockey player statistics, goal leaders, and analytics for the ${SEASON_SHORT} season.`}
        />
        <meta
          property="og:title"
          content="ASU Hockey Player Stats & Leaders | Forks Up Pucks"
        />
        <meta
          ...
          {JSON.stringify({
            ...
            itemListElement: [
              ...
              },
              {
                ...
                position: 2,
                name: "Stats",
                item: "https://forksuppucks.com/stats/",
              },
            ],
          })}
        </script>
