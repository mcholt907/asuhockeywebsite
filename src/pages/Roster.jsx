...
function Roster() {
  ...
  const players = useMemo(() => {
    ...
    return data.filter(
      (p) =>
        p && p.name && p.number && /^#?[0-9]+$/.test(String(p.number).trim()),
    );
  }, [data]);
  ...
  const playersByPosition = useMemo(
    () => ({
      ...
      forwards: players.filter(
        (p) => !isGoalie(p) && !isDefenseman(p) && p.name,
      ),
    }),
    [players],
  );
  ...
  const filteredPlayers = useMemo(() => {
    ...
    if (selectedPosition === "g")
      return { ...EMPTY_GROUPS, goaltenders: playersByPosition.goaltenders };
    if (selectedPosition === "d")
      return { ...EMPTY_GROUPS, defensemen: playersByPosition.defensemen };
    if (selectedPosition === "f")
      return { ...EMPTY_GROUPS, forwards: playersByPosition.forwards };
  ...
  return (
    <div className="page-container roster-page">
      <Helmet>
        <title>{`ASU Hockey Team Roster ${SEASON_FULL} | Forks Up Pucks`}</title>
        <meta
          name="description"
          content={`${SEASON_SHORT} ASU Sun Devils Men's Hockey team roster. Find players, positions, jersey numbers, and hometowns.`}
        />
        <meta
          property="og:title"
          content={`ASU Hockey Team Roster ${SEASON_FULL} | Forks Up Pucks`}
        />
        <meta
                ...
                })),
                {
                  ...
                  itemListElement: [
                    ...
                    },
                    {
                      ...
                      position: 2,
                      name: "Roster",
                      item: "https://forksuppucks.com/roster/",
                    },
                  ],
                },
              ],
      ...
      <div className="page-header">
        <h1>{`ASU Hockey Roster (${SEASON_FULL} Season)`}</h1>
        <PositionFilter
      ...
      <div className="roster-content">
        <RosterPositionGroup
          title="Goaltenders"
          players={filteredPlayers.goaltenders}
          keyPrefix="g"
        />
        <RosterPositionGroup
          title="Defensemen"
          players={filteredPlayers.defensemen}
          keyPrefix="d"
        />
        <RosterPositionGroup
          title="Forwards"
