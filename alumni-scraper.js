...
async function scrapePage(url, type) {
  ...
  $("table tbody tr").each((i, row) => {
    ...
    const numericCells = cellTexts
      .filter((t) => {
        ...
        return (
          /^\d+$/.test(val) || // Pure integers: 35, 17, 0
    ...
    if (playerLink.length > 0) {
      ...
      const playerUrl = playerHref.startsWith("http")
        ? playerHref
        : `https://www.eliteprospects.com${playerHref}`;
      ...
      if (teamLinks.length > 0) {
        ...
        const teamUrl = teamHref.startsWith("http")
          ? teamHref
          : `https://www.eliteprospects.com${teamHref}`;
        ...
        if (leagueLinks.length > 0) {
          ...
          leagueUrl = leagueHref.startsWith("http")
            ? leagueHref
            : `https://www.eliteprospects.com${leagueHref}`;
        }
  ...
  console.log(
    `[Alumni Scraper] Found ${entries.length} ${type} entries on this page`,
  );
...
async function scrapeAllPages(type) {
  const allEntries = [];
  let url = type === "goalies" ? `${BASE_URL}?tab=goalies` : BASE_URL;
  ...
  console.log(
    `[Alumni Scraper] Total ${type} entries: ${allEntries.length} (from ${pageCount} pages)`,
  );
  return allEntries;
}
